...
class OST_train(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        w, h = raw_image.size
        if w < self.crop_size:
            raw_image = raw_image.resize((self.crop_size, h), resample=Image.BICUBIC)
            annotation = annotation.resize((self.crop_size, h), resample=Image.NEAREST)
        w, h = raw_image.size
        if h < self.crop_size:
            raw_image = raw_image.resize((w, self.crop_size), resample=Image.BICUBIC)
            annotation = annotation.resize((w, self.crop_size), resample=Image.NEAREST)
        ...
        return noisy_image_seg, image_part, noisy_image_seg.shape
...
class OST_test(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image_part, image_part, image_part.shape
